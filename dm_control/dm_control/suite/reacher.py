...
class Reacher(base.Task):
    ...
    """
        ...
        self._obstacle_size = 0.05  # TODO [name redacted :)]: make this configurable later

        super().__init__(random=random)

        self.target_angle = -.25 * np.pi
        self.target_radius = 0.25

        self.constraint_angle = -.3 * np.pi
        self.constraint_radius = 0.13

    def initialize_episode(self, physics, random_start=False):
        ...
        if not random_start:
            physics.named.data.qpos['shoulder'] = np.pi * .53
            physics.named.data.qpos['wrist'] = 0.85 * np.pi
        else:
            physics.named.data.qpos['shoulder'] = np.random.uniform(-np.pi, np.pi)
            physics.named.data.qpos['wrist'] = np.random.uniform(-np.pi * 0.85, np.pi * .85)

        # Randomize target position
        # angle = self.random.uniform(0, 2 * np.pi)
        # radius = self.random.uniform(.15, .20)
        physics.named.model.geom_pos['target', 'x'] = self.target_radius * np.sin(self.target_angle)
        physics.named.model.geom_pos['target', 'y'] = self.target_radius * np.cos(self.target_angle)
        ...
        physics.named.model.geom_pos['obstacle', 'x'] = self.constraint_radius * np.sin(
            self.constraint_angle)
        physics.named.model.geom_pos['obstacle', 'y'] = self.constraint_radius * np.cos(
            self.constraint_angle)
