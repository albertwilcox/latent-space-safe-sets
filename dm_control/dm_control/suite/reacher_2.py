...
def get_model_and_assets():
  ...
  return common.read_model('reacher_2.xml'), common.ASSETS
...
class Reacher(base.Task):
  ...
  def __init__(self, target_size, random=None):
    ...
    self.target_angle = self.random.uniform(0, 2 * np.pi)
    self.target_radius = self.random.uniform(.15, .20)

    self.constraint_angle = self.random.uniform(0, 2 * np.pi)
    self.constraint_radius = self.random.uniform(.05, .1)
  ...
  def initialize_episode(self, physics):
    ...
    physics.named.model.geom_pos['obstacle', 'x'] = self.constraint_radius * np.sin(self.constraint_angle)
    physics.named.model.geom_pos['obstacle', 'y'] = self.constraint_radius * np.cos(self.constraint_angle)
